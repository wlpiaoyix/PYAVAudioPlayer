...
@interface PYAVAudioPlayer()<AVAudioPlayerDelegate>{
}
@end
@implementation PYAVAudioPlayer
-(id) init{
    if (self=[super init]) {
        _arrayAudiosURL = [NSMutableArray new];
        _playerStatus = PYAVAudioPlayerStatusPrepare;
        [PYAVAudioTools backgourndPlay:YES];
    }
    return self;
}
...
-(BOOL) play{
    @synchronized(self) {
        if (self.playerStatus == PYAVAudioPlayerStatusPlay) {
            return false;
        }
        if (![self __playProgress:DEFAULT_PROGRESS]) {
            return false;
        }
        _playerStatus = PYAVAudioPlayerStatusPlay;
    }
    return true;
}
...
-(BOOL) pause{
    @synchronized(self) {
        ...
        [self.player pause];
        _playerStatus = PYAVAudioPlayerStatusPause;
    }
    return true;
}
-(BOOL) stop{
    @synchronized(self) {
        ...
        [self.player stop];
        _player = nil;
        _playerStatus = PYAVAudioPlayerStatusStop;
    }
    return true;
}
-(BOOL) next{
...
-(BOOL) __playProgress:(CGFloat) progress{
    if (!self.player) {
        ...
        if (url) {
            _audioInfo = [PYAVAudioTools getAudioInfoByUrl:url];
    ...
    if (!self.player.isPlaying) {
        [self.player play];
        [PYAVAudioTools configNowPlayingInfoCenter:self.audioInfo player:self.player];
    }
    return true;
}
-(void) dealloc{

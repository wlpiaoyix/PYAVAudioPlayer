...
NSMapTable<NSString *, id> *PYAVAudioMapTable;

@interface PYAVAudioTools()

@end

void _hook_remoteControlReceivedWithEvent_(id target, SEL action, UIEvent * receivedEvent) {
    id<PYAudioPlayer> player = [PYAVAudioMapTable objectForKey:@"player"];
    if (!player) {
        return;
    }
    [PYAVAudioTools remoteControlReceivedWithEvent:receivedEvent player:player];
    
    if ([((NSObject*)target) respondsToSelector:sel_getUid("_hookremoteControlReceivedWithEvent:")]) {
        [PYReflect invoke:target action:sel_getUid("_hookremoteControlReceivedWithEvent:") returnValue:nil params:&receivedEvent, nil];
    }
}
...
@implementation PYAVAudioTools
+(void) initialize{
    PYAVAudioMapTable = [NSMapTable weakToStrongObjectsMapTable];
}
        ...
        switch (receivedEvent.subtype) {
            ...
            case UIEventSubtypeRemoteControlTogglePlayPause:{
                switch (player.playerStatus) {
                    case PYAVAudioPlayerStatusPlay:{
                        [player pause];
                    }
                        break;
...
+(void) hookremoteControlReceivedWithPlayer:(id<PYAudioPlayer> _Nonnull) player{
    ...
    [PYAVAudioMapTable setObject:player forKey:@"player"];
